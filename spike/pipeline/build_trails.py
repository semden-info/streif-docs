...
usage:
    python build_trails.py RAW.xml OUT.geojson [--min-len=75] [--target=200] [--keep-unnamed]
                           [--osm=PATH] [--tettsteder=PATH] [--kommuner=1577,1520] [--cache=DIR]
"""
import io
import json
import math
...
# ── ОДИНИЦЯ МАРШРУТУ: комуна (правка 1 Дениса, 2026-08-07) ───────────────────────────────────────
#
# Блок стежок у шторці статистики мусить залежати від обраної одиниці — так само, як об'єкти за
# типами. Доти він цього не міг: у `trails.geojson` не було тегів, і підзаголовок чесно казав
# «uavhengig av området over». Тепер тег є.
#
# ⚠️ **Одиниця для стежок — КОМУНА, і поселення тут не альтернатива, а неможливість.** Фільтр
# «лише поза містом» (вище) викидає маршрут тоді й лише тоді, коли більша частина його довжини в
# поселенні; правило належності — та сама більша частина. Отже маршрут, який дістав би тег
# поселення, уже викинуто: під вкладкою «Tettsted» стежок не нуль «майже завжди», а нуль ЗАВЖДИ,
# за побудовою. Зміряно на живому наборі: маршрутів із більшістю в поселенні — 0; заходять у
# поселення хоч трохи — 8 із 89, і це 1,7 км зі 160,3.
#
# ⚠️ **Сусідні комуни доганяємо автоматично** (`discover`). bbox джерела — прямокутник із запасом,
