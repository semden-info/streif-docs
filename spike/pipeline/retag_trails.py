...
def main():
    ...
    # ⛔ **ГЕЙТ ЦІЛІСНОСТІ** (знахідка review 6). Раніше скрипт віддавав код 0 навіть тоді, коли
    # частина маршрутів лишилась без комуни — а клієнт трактує порожній тег не як «дані ще їдуть»,
    # а як невидимий у шторці прогрес. Найпростіший спосіб отримати такий файл — недоступний
    # `kommuneinfo/punkt` у момент збірки: жодної помилки, просто тихо гірший результат.
    # Ненульовий код — це те, що зупинить публікацію; текст поруч зупинити не міг.
    lost = report.get("")
    if lost or split:
        print()
        if lost:
            print("⛔ %d маршрутів (%d ділянок, %.1f км) БЕЗ КОМУНИ — у шторці їх не буде видно."
                  % (lost["routes"], lost["segments"], lost["km"]))
        if split:
            print("⛔ %d маршрутів дістали різні комуни на різних ділянках." % len(split))
        print("   Файл записано (щоб було що дивитись), але публікувати його НЕ МОЖНА.")
        sys.exit(2)
