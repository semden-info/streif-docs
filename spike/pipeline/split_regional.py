...
# Довідник фюльке — потрібен лише для ПІДПИСУ груп у списку завантажень; код фюльке беремо
# з kommunenummer, тож без цього запиту все працює, просто групи звуться числом.
FYLKER_URL = "https://ws.geonorge.no/kommuneinfo/v1/fylker"
...
def main():
    ...
    # ── довідник фюльке: два перші символи kommunenummer і є його кодом ──────────────────────────
    #
    # ⚠️ **Це задум наперед, а не сьогоднішня потреба** (побажання Дениса 2026-08-08). Даних у нас
    # поки на одне фюльке, тож групування зараз нічого не змінює візуально. Але щойно з'явиться
    # друге, список має вже вміти «розкрий фюльке → обери комуни або взяв усе»; додати поле в індекс
    # зараз коштує один запит, а потім коштувало б перевипуску індексу в усіх, хто вже його має.
    fylker = {}
    try:
        for f in json.loads(geo_units.fetch(FYLKER_URL, os.path.join(cache, "fylker.json")).decode("utf-8")):
            fylker[str(f.get("fylkesnummer", "")).zfill(2)] = f.get("fylkesnavn", "")
        print("фюльке в довіднику: %d" % len(fylker))
    except Exception as e:
        # Не блокер: без назв групи підпишуться кодом, і це гірше лише естетично.
        print("⚠️ довідник фюльке недоступний (%s) — групи підпишуться кодом" % e)
    ...
    for c in codes:
        ...
        index.append({
            ...
            # Код фюльке — це просто два перші символи kommunenummer (1577 → 15). Окремого джерела
            # не треба, і саме тому поле дешеве; назва — з довідника вище.
            "fylke": c[:2],
            "fylkeName": fylker.get(c[:2], ""),
