...
## Версія схеми тайлів — `manifest.dataVersion`

**Навіщо.** Клієнт кешує зони **назавжди** (D24 «раз стягнули — назавжди», `filesDir/areas`). Коли ми
міняємо СКЛАД властивостей у тайлі, старий кеш мовчки лишається — і фіча просто не працює: саме так
`bt` виходив порожнім, а Coverage по комуні показував нуль. Кеш доводилось чистити руками через `adb`;
тестери цього не робитимуть і мовчки бачитимуть застарілі дані. `dataVersion` — єдиний сигнал
«формат змінився, перезавантаж зони».

**Це не `generated`.** `generated` — час збірки, він міняється щоребілду. Якби кеш скидався по ньому,
кожен реген тих самих даних коштував би повторного завантаження всіх зон усім користувачам.
`dataVersion` описує **формат і склад властивостей**, а не свіжість.

| Підіймати `DATA_VERSION` | НЕ підіймати |
|---|---|
| додано / перейменовано / прибрано властивість фічі (напр. `bt`) | перезібрали ті самі дані (новий `generated`) |
| змінилась семантика наявної (інша шкала, інший id-простір `building_id`) | додали або прибрали комуни з регіону |
| змінилась сітка тайлів (`TILE`) | змінились лише лічильники в manifest |
| | оновився Matrikkelen / OSM без зміни полів |

**Де живе:** константа `DATA_VERSION` угорі `build_tiles.py` → пишеться в `manifest.json` і в лог збірки
(`manifest: region=… dataVersion=2 …`). Історія: **1** — `building_id`/`type`/`accessible`
(+`tettsted_id` P20, `kommune` P30/P31) · **2** — `+ bt` (сирий bygningstype, P28).

**Як це бачить клієнт** (`DataVersion.kt`, `AreaCache`, `AreaLoader`): кеш зон лежить у теці, що
**іменована версією** — `filesDir/areas/v2/`. Зміна версії = перехід у нову теку (атомарно, без
ризику змішати схеми при обриві) + прибирання старих `v*`. Розкриття живуть у Room і НЕ чіпаються
(D30) — після перезавантаження зон вони відновлюються звичайним `reconcilePersisted`.
Інвалідація спрацьовує **лише на реальній зміні числа**, не на кожен старт; manifest без `dataVersion`
(або з `0`) не інвалідує нічого.

⚠️ Якщо підняв `DATA_VERSION` — **перезбери й залий ВЕСЬ регіон**, не окремі тайли: клієнт після
переходу викачує зони заново й має отримати однорідну схему.
