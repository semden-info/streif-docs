...
> **Статус:** 🗄️ **АРХІВ — review ВИКОНАНО 2026-07-01** (вердикт «готово до MVP-0 з умовами»). Підсумок → `CHANGELOG.md` (2026-07-01) + `DECISIONS.md` **D23/D23.1**. Лишається як мапа коду / run-plan-шаблон для майбутніх review.
...
**Gate перед MVP-0.** Spike-2 майже завершено (Stage A/B ✅, C 🛠️ battery); поверх — on-demand (D24) і edge-distance матчинг (D25), уточнений у D25.1 до edge-eligibility + centroid-closest-approach. Перед тим як будувати MVP-0 (Room, eligibility-фільтр, onboarding, дизайн), треба **незалежно перевірити фундамент**: код, архітектуру, оптимальність тех-стеку.
