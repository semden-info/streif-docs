...
**✅ Device-верифіковано на Pixel (48290DLAQ003FR, реальні дані):** OSM-збудований `tettsteder.geojson` (Volda 6071 = **3686 буд. / 3576 accessible** vs комуна 18809 — саме розв'язує P18-«завелику area»; 10188/22645 буд. у tettsteder, решта — сільське) залито в `filesDir`-кеш. Launch без крешу, парс 800KB, **GPS→locate=Volda → плашка «0,0% of Volda»** (глобальний лічильник 190→222, % per-tettsted), P18-фолбек у спокої «1,0% of area», **© SSB** атрибуція. `numerator=0` бо CDN-тайли ще без `tettsted_id`; **boundary-візуал + реальні числа — після регену+walk Дениса** (⚠️ очистити `filesDir/areas` — D24 «раз стягнули — назавжди»). Пайплайн-логіку, Kotlin-юніт-тести (`TettstedIndexTest`) й компіляцію теж верифіковано. Реалізує `P20` (розширює `P18`, закриває `P3`).
