...
- **P19** Обʼєм — ціль-так/формула-ні (баланс через complementary-колекції + бейдж «Balansert utforsker»; мультиплікативна формула відхилена — вироджений випадок «місто-only → 0» несправедливо занулює тих, хто збирає лише один тип об'єктів) · **P20** діорама 2D-first (справжнє 3D лише після прототипу+retention; native-OOM ризик D32) · **P21** типи-як-точки-дій (гард: без хард-гейту за рідкісним типом, тактовно/безпечно) · **P22** новизна маршруту (інструментує ★-метрику D2; залежить від P6) · **P23** історичний шар (Riksantikvaren/Nasjonalbiblioteket/Kartverket historiske). Ідея «**3 герої**» — відкладено (буквальні ваги = score + ризик колії проти D2).
