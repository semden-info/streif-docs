...
**Алгоритм (як реалізовано, `BuildingStore`/`TrackingRepository`):**
- **Відстань = GPS → контур полігона** (point-to-ring; точка всередині footprint → 0). Локальна планарна проєкція **за широтою запиту** (коректно по всій Норвегії).
...
- **closest-approach gate:** розкрити, коли edge-відстань **виросла на `SLACK` над мінімумом** (= проминув найближчу точку, реально йшов уздовж), **АБО** стіна ≤ `MIN_EDGE` (впритул-байпас). Так будинки попереду НЕ засвічуються зарано, а через дорогу — ловляться. **Без abreast-курсу** (adversarial-рев'ю показало: курс на короткій базі шумний → крихкий; closest-approach course-free, помиляється «пізніше»).
- **Параметри (тунабельні, поточні):** `R_FAR=18 м` · `MIN_EDGE=8 м` · `SLACK=1.5 м` · `ACC_MAX=30 м` (fail-closed). Польова верифікація на тому ж треку: recall 69→76/83, edge@reveal 19→10 м.
...
- **Eligibility (D6):** рахувати лише будинки, доступні з публічної пішої мережі — **ще НЕ реалізовано, на MVP-0**.

**Тунабельні параметри** (фіналізуються як acceptance criteria в `09`, `DECISIONS.md` P2): `R_FAR` (18↔20 на ширші вулиці) · `MIN_EDGE` · `SLACK` · `ACC_MAX`. Тюнінг — через ✓/✗-маркування (debug) + `spike/fieldtest/analyze.py` на польових логах.
