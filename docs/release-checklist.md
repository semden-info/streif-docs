...
| # | Що | Стан |
|---|---|---|
| T1 | Модуль збору + перемикач у Settings + **екран згоди при першому запуску** (default-ON) | 🔴 не зроблено |
| T2 | Cloudflare Worker → R2 (ndjson) + скрипт аналізу поруч із `spike/fieldtest/analyze.py` | 🔴 не зроблено |
